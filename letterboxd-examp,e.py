...
def getReviews(user):
    ...
    for url in reviewUrls:
        ...
        valuableStart = htmlText.find('<div class="film-detail-content">')
        print('hiiiiiiiii')
        print(valuableStart)
        print('hiiiiiiiii')
        # para = valuableStart.find('<p>')
        # print(para)
        ...
        paraStart = smallStr.find('<p>')
        paraEnd = smallStr.find('</p>', paraStart)
        para = smallStr[paraStart:paraEnd]
        print(para)

        for line in [line for line in smallStr.splitlines() if line.strip()[10:24] =='"film-detail-content">']:
